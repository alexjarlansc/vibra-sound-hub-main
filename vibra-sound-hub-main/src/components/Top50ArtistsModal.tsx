...
export default function Top50ArtistsModal({ open, onOpenChange }: { open: boolean; onOpenChange: (o:boolean)=>void }){
  ...
  if(import.meta.env.DEV){ try{ console.debug('[Top50ArtistsModal] profiles', profiles); }catch(e){} }

  // visible debug area in dev to show avatar_url values
  const DevProfilesDebug = () => {
    if(!import.meta.env.DEV || !profiles || !profiles.length) return null;
    return (
      <div className="my-2 p-2 bg-yellow-50 text-xs text-muted-foreground rounded">
        <div className="font-medium">DEV DEBUG: profiles</div>
        {profiles.map((p:any, i:number)=> (
          <div key={p.id || i} className="truncate">{i+1}. {p.username || p.name} — avatar_url: <span className="text-emerald-700">{String(p.avatar_url)}</span></div>
        ))}
      </div>
    );
  };
